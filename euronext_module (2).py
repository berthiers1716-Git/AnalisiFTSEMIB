    ...
    if os.path.exists(log_path):
        ...
        for col in _LOG_COLS:
            if col not in existing.columns:
                existing[col] = 'in attesa' if col == 'stato_conferma' else (True if col == 'significativo' else np.nan)
...
def reconcile_log(log_path, dati_folder, confirm_ratio=0.30):
    ...
    for col in _LOG_COLS:
        if col not in log_df.columns:
            log_df[col] = 'in attesa' if col == 'stato_conferma' else (True if col == 'significativo' else np.nan)
    ...
    for idx, row in log_df.iterrows():
        ...
        needs_confirm = pd.isna(row.get('stato_conferma')) or row.get('stato_conferma') in (None, '', 'in attesa')
        if needs_confirm:
...
def update_significativo_flags(log_path, edited_subset):
    """
    Aggiorna solo la colonna 'significativo' nel log su disco, in base a un
    sottoinsieme modificato (es. quello mostrato/editato nell'interfaccia, anche
    se filtrato per data o stato). Il merge avviene per chiave
    (data_riferimento, scadenza, strike, tipo), cosi' le righe filtrate/nascoste
    nella vista non vengono perse.

    Returns: (n_modificate, df_log_completo_aggiornato)
    """
    if not os.path.exists(log_path):
    ...
    mapping = dict(zip(edited['_key'], edited['significativo']))

    n_changed = 0
    for idx, row in log_df.iterrows():
        if row['_key'] in mapping and bool(row['significativo']) != bool(mapping[row['_key']]):
            log_df.at[idx, 'significativo'] = bool(mapping[row['_key']])
            n_changed += 1
