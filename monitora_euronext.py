...
def main():
    ...
    parser.add_argument('--batch', type=int, default=DEFAULT_BATCH_SIZE,
                         help=f"Quante scadenze richiedere per chiamata (default {DEFAULT_BATCH_SIZE}, "
                              "riduci se il server va in timeout 504)")
    parser.add_argument('--data', type=str, default=None,
                         help="Data di negoziazione da richiedere, formato MM-DD-YYYY (default: oggi). "
                              "Serve per testare il recupero di date passate, es. --data 07-10-2026")
    args = parser.parse_args()

    try:
        session = crea_sessione()
        html_parts = fetch_live_html_batched(
            session, expiries=args.scadenze or SCADENZE_NOTE, trade_date=args.data,
            batch_size=args.batch, verbose=True
        )
        for i, h in enumerate(html_parts):
            print(f"[DEBUG] Blocco {i}: {len(h)} caratteri ricevuti")
        ...
        if 'html_parts' in dir():
            for i, h in enumerate(html_parts):
                print(f"\n[DEBUG] --- Blocco {i} (primi 2500 caratteri) ---", file=sys.stderr)
                print(h[:2500], file=sys.stderr)
                print(f"\n[DEBUG] --- Blocco {i}: repr dei primi 400 caratteri (per vedere spazi/caratteri nascosti) ---", file=sys.stderr)
                print(repr(h[:400]), file=sys.stderr)
        sys.exit(1)
