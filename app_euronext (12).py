...
if df_raw is not None and spot_price > 0:
    ...
    with st.expander("📈 Storico Spot / Volume / OI totali — per verificare la correlazione", expanded=False):
        if len(storico_totali) >= 2:
            ...
            st.caption(
                "Tre pannelli allineati sulla stessa data, ciascuno con la propria scala (l'OI è di ordini di "
                "grandezza più alto del Volume daily, quindi non si leggono bene su un asse condiviso). "
                "Confronta se un salto di OI o Volume coincide con un movimento marcato dello spot in alto."
            )
        else:
