...
from euronext_module import (
    ...
    reconcile_log, update_significativo_flags
)
...
file caricato), così non restano in mezzo scadenze ormai concluse.
        ...
        if _log_df is None and os.path.exists(VOLUME_LOG_PATH):
            ...
            if 'significativo' not in _log_df.columns:
                _log_df['significativo'] = True
        ...
        if _log_df is None or _log_df.empty:
            ...
            if _view.empty:
                ...
                _other_cols = [c for c in _view.columns if c not in _key_cols + ['significativo']]
                _edited = st.data_editor(
                    _view[_key_cols + ['significativo'] + _other_cols],
                    column_config={
                        "significativo": st.column_config.CheckboxColumn(
                            "Significativo", help="Deseleziona per marcare la riga come non rilevante"
                        ),
                        **{c: st.column_config.Column(disabled=True) for c in _key_cols + _other_cols}
                    },
                    hide_index=True, width="stretch", key="log_editor"
                )
                if st.button("Salva modifiche ai flag"):
                    n_changed, updated_log = update_significativo_flags(
                        VOLUME_LOG_PATH, _edited[_key_cols + ['significativo']]
                    )
                    if n_changed > 0:
            ...
            st.caption(
                "Il flag 'Significativo' resta salvato nel CSV anche filtrando la vista: puoi "
                "deselezionare/riselezionare una riga in qualunque momento, anche dopo settimane."
            )
