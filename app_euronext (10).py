...
if uploaded_file is not None:
    try:
        ...
        storico_totali = log_totali_giornalieri(df_raw, analysis_date, TOTALI_LOG_PATH)
        ...
        with st.expander("📈 Storico Volume/OI totali (tutte le date registrate)", expanded=False):
            if len(storico_totali) >= 2:
                st.line_chart(storico_totali.set_index('data')[['oi_totale', 'volume_totale']])
                st.caption(
                    "OI totale fermo/laterale = mercato senza grandi variazioni di posizionamento aggregato; "
                    "salti evidenti da un giorno all'altro meritano un'occhiata più da vicino."
                )
            else:
