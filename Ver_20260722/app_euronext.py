...
# -----------------------------------------------------------------------------
# CARICAMENTO DATI: incolla il testo Euronext (niente upload file)
# -----------------------------------------------------------------------------
st.subheader("1. Incolla i dati Euronext")
st.caption(
    "Vai sulla pagina delle opzioni FTSEMIB su Euronext/Borsa Italiana, seleziona e copia "
    "l'intera tabella (tutte le scadenze che ti interessano), incollala qui sotto. "
    "Usa un export con Open Interest già disponibile (di solito il giorno successivo)."
)
pasted_text = st.text_area("Dati incollati", height=200, placeholder="10 July 2026\n\nJul 2026 Prices - 10 July 2026\nStrike\tType\tOpen\t...")

df_raw, analysis_date = None, None
if pasted_text.strip():
    try:
        df_raw, analysis_date = parse_euronext_text(pasted_text)
        st.success(f"Letto: {len(df_raw)} righe, data di riferimento {analysis_date.date()}.")
    except Exception as e:
        st.error(f"Errore nel parsing del testo incollato: {e}")
else:
    st.info("In attesa dei dati incollati...")
