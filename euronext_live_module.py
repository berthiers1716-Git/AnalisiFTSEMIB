...
_HEADER_RE = re.compile(
    r'<h3>(\w{3} \d{4}) Prices (?:as of (\d{1,2} \w+ \d{4}) (\d{2}:\d{2}) CET'
    r'|- (\d{1,2} \w+ \d{4}))</h3>'
)
_ROW_RE = re.compile(
...
def fetch_live_html_batched(session, expiries=None, trade_date=None, batch_size=5, verbose=False):
    """
    Come fetch_live_html, ma spezza la richiesta in blocchi da 'batch_size'
    scadenze (di default 5, come nel test manuale nel browser), per evitare
    di sovraccaricare il server Euronext con tutte le scadenze insieme (osservato:
    la richiesta con tutte le 14 scadenze in un colpo solo va in timeout 504
    lato server; a blocchi piu' piccoli funziona).

    Returns: lista di testi HTML (uno per blocco), da passare singolarmente a
    parse_live_html e concatenare i risultati.
    """
    if expiries is None:
        expiries = SCADENZE_NOTE

    html_parts = []
    for i in range(0, len(expiries), batch_size):
        chunk = expiries[i:i + batch_size]
        if verbose:
            print(f"Richiesta scadenze {chunk}...")
        html_parts.append(fetch_live_html(session, expiries=chunk, trade_date=trade_date))
    return html_parts
...
def parse_live_html(html_text):
    ...
    for i, h in enumerate(headers):
        mon_year_str, date_with_time_str, time_str, date_only_str = h.groups()
        # Formato A ("... as of DD Mon YYYY HH:MM CET"): scadenza con attivita' oggi.
        # Formato B ("... - DD Mon YYYY"): scadenza poco/non negoziata oggi, niente orario.
        if date_with_time_str is not None:
            date_str = date_with_time_str
        else:
            date_str = date_only_str
            time_str = ''  # nessun orario disponibile per questo blocco

        mon_abbr, year = mon_year_str.split()
        year = int(year)
        month_num = dt.datetime.strptime(mon_abbr, "%b").month
        update_date = pd.to_datetime(date_str).date()
        ...
        for m in _ROW_RE.finditer(segment):
            strike_s, typ, _o, _h_, _l, last_s, _chg, settle_s, vol_s, oi_s = m.groups()
            try:
                strike = float(strike_s)
            except ValueError:
                continue

            # Il Settle ufficiale intraday e' spesso "N/A" (si calcola a fine giornata,
            # come l'Open Interest): in tal caso si usa l'ultimo prezzo scambiato (Last)
            # come alternativa, se disponibile. Se nessuno dei due e' valido, resta NaN
            # (la riga viene comunque tenuta: serve comunque per Volume/OI).
            settle_clean = settle_s.strip()
            last_clean = last_s.strip()
            if settle_clean not in ('', 'N/A'):
                settle = float(settle_clean)
            elif last_clean not in ('', 'N/A'):
                settle = float(last_clean)
            else:
                settle = np.nan
