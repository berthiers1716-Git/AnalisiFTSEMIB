...
from euronext_module import (
    ...
    reconcile_log, update_log_fields, log_totali_giornalieri
)
...
if df_raw is not None and spot_price > 0:
    storico_totali = log_totali_giornalieri(df_raw, analysis_date, TOTALI_LOG_PATH, spot=spot_price)
    _oggi = storico_totali[storico_totali['data'] == analysis_date.date().isoformat()].iloc[0]
    col_t1, col_t2, col_t3 = st.columns(3)
    col_t1.metric("Volume totale (tutte le scadenze)", f"{_oggi['volume_totale']:,.0f}")
    col_t2.metric("Open Interest totale (tutte le scadenze)", f"{_oggi['oi_totale']:,.0f}")
    col_t3.metric("Spot registrato", f"{_oggi['spot']:,.2f}" if pd.notna(_oggi['spot']) else "N/A")

    with st.expander("📈 Storico Spot / Volume / OI totali — per verificare la correlazione", expanded=False):
        if len(storico_totali) >= 2:
            _fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.08,
                                  subplot_titles=("Spot FTSEMIB", "Volume e Open Interest totali"))
            _fig.add_trace(go.Scatter(x=storico_totali['data'], y=storico_totali['spot'],
                                       mode='lines+markers', name='Spot', line=dict(color='#60a5fa')), row=1, col=1)
            _fig.add_trace(go.Scatter(x=storico_totali['data'], y=storico_totali['oi_totale'],
                                       mode='lines+markers', name='OI totale', line=dict(color='#34d399')), row=2, col=1)
            _fig.add_trace(go.Scatter(x=storico_totali['data'], y=storico_totali['volume_totale'],
                                       mode='lines+markers', name='Volume totale', line=dict(color='#f97316')), row=2, col=1)
            _fig.update_layout(height=500, template='plotly_dark', margin=dict(l=10, r=10, t=40, b=10))
            st.plotly_chart(_fig, width="stretch")
            st.caption(
                "Due pannelli allineati sulla stessa data: confronta se un salto di OI/Volume nel pannello "
                "in basso coincide con un movimento marcato dello spot in quello sopra. OI fermo/laterale "
                "in genere coincide con un mercato senza grandi variazioni di posizionamento."
            )
        else:
            st.info("Ancora una sola data registrata: il grafico comparirà da quando ne avrai almeno due.")
        st.dataframe(storico_totali.sort_values('data', ascending=False), width="stretch", hide_index=True)
...
if df_raw is not None and spot_price > 0:
    ...
    tab_summary, tab_gex, tab_vex_dex, tab_oi_vol, tab_stats, tab_vol_surf, tab_eventi = st.tabs([
