...
with st.sidebar:
    ...
    spot_price = st.number_input(
        ...
        help="Precompilato automaticamente se carichi lo storico prezzi (punto 2 sopra); "
             "altrimenti va inserito a mano (es. chiusura del giorno)."
    )
    st.divider()
...
**⚠️ Attenzione.** Questa tab si basa **solo su OI e Volume**, dati reali forniti direttamente da Euronext (non stimati) — a differenza delle tab GEX/VEX/DEX, qui non c'è alcuna derivazione Black-Scholes di mezzo. I Wall restano comunque concentrazioni di OI, non muri garantiti.
        ...
        st.markdown("#### Volumi totali della scadenza (valori assoluti)")
        colv1, colv2, colv3 = st.columns(3)
        colv1.metric("Volume Call (totale)", f"{_total_call_vol:,.0f}")
        colv2.metric("Volume Put (totale)", f"{_total_put_vol:,.0f}")
        colv3.metric("Volume Totale (Call+Put)", f"{_total_call_vol + _total_put_vol:,.0f}")
        st.caption(
            "Somma di TUTTA la catena per questa scadenza (non solo la fascia ±25% intorno "
            "allo spot mostrata nel grafico sotto). Utile su un indice come il FTSEMIB, dove "
            "i volumi assoluti sono molto più contenuti che su SPX: un P/C Ratio alto su pochi "
            "contratti pesa diversamente da uno alto su migliaia."
        )
